...
class Pile
  ...
  # not valid.
  def play(card)
    raise ArgumentError.new "must declare suit when playing eight" if card.value == :eight
    if valid_play?(card) 
      @top_card = card 
      @current_suit = card.suit
    else 
      raise ArgumentError.new "invalid play"
    end
  end
  ...
  def play_eight(card, suit_choice)
    if card.value == :eight 
      ...
      raise ArgumentError.new "must play eight"
    end
  end
end
